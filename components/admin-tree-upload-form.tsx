    ...
    <form action={action} className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs leading-6 text-stone-500">
          普通目录只上传文章文件。图片请进入对应的 `resource` 目录单独上传。
        </p>
      </div>
...
function ResourceUploadForm({ targetPath }: { targetPath: string }) {
  const [state, action, pending] = useActionState(
  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs leading-6 text-stone-500">
          选中 `resource` 目录后支持多张图片上传，并按图片格式进行服务端校验。
        </p>
      </div>
...
export function AdminTreeUploadForm(props: AdminTreeUploadFormProps) {
  if (props.mode === "resource") {
    return <ResourceUploadForm targetPath={props.targetPath} />;
  }
